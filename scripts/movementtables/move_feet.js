var Constructor = function()
{
    ...
    this.getMovementpoints = function(terrain, unit, currentTerrain, trapChecking = false, map)
    {
        ...
        if(id === "TRENCH") {
            if(Global.getValidDirectionsFromTarget(currentTerrain,terrain) === false) {
                return 2;
            }
        }
        if(currentTerrain.getID() === "TRENCH") {
            if(Global.getValidDirectionsFromHere(currentTerrain,terrain) === false) {
                if(MOVEMENTTABLE.getMovementpointsFromTable(terrain, MOVE_FEET.movementpointsTable) > 0) {
                    return MOVEMENTTABLE.getMovementpointsFromTable(terrain, MOVE_FEET.movementpointsTable) +1;
                }
            }
        }
