var Constructor = function()
{
    ...
    {
        return qsTr("A player loses when he loses his last HQ or Command Vehicle. All buildings will be owned by the current player.")
    };
    ...
    this.init = function(rule, map)
    {
        ...
        for (var i = 0; i < playerCount; i++)
        {
            ...
            if (player.getBuildingListCount(BUILDING.hqIds) > 0)
            {
    ...
    this.checkHQCount = function(rule, player, map)
    {
        ...
        if (value === false)
        {
            if (player.getBuildingListCount(BUILDING.hqIds) > 0 && player.getUnitCount("FW_COMV") > 0)
            {
                variable.writeDataBool(true);
                value = true;
            }
